...
| 模块 | 页面 | 主要能力 |
| --- | --- | --- |
| 养护总览 | `/dashboard` | 绿地与养护总量指标、近半年记录与工时趋势、绿地类型/任务类型/更换原因分布、任务提醒（逾期与临期）、养护工作量排名 |
...
| GET | `/statistics/dashboard` | 看板聚合数据（总览 + 分布 + 趋势 + 榜单 + 任务提醒 + 最近动态） |
| GET | `/statistics/overview` `/distributions` `/trends` `/ranking` `/reminders` | 看板分项接口（`reminders` 返回按统一规则排序的逾期与临期任务清单） |
...
7. **任务提醒与排序**：提醒周期按优先级区分（紧急提前 7 天、高 5 天、中 3 天、低 1 天）；未办结任务按计划日期与提醒周期分为「已逾期 → 今日到期 → 临期 → 未到期」四级，排序先按提醒级别、同级内按计划日期升序、再按优先级（紧急优先）与编号排列，紧急且临期的任务排在前面，已办结任务不参与提醒并排在列表最后。任务列表默认排序、`/statistics/reminders` 提醒清单与看板任务排名共用同一套规则（`services/task_reminder.py`），列表仍可用 `sort`/`order` 参数显式指定排序字段。
...
```bash
cd backend
python -m pytest              # 56 个用例：接口、校验、跨模块规则、端到端流程
```

覆盖重点：绿地编号生成与唯一性、枚举与字段校验、列表过滤/排序/分页、任务状态自动流转与手动流转限制、记录删除后的状态回退、更换金额核算、删除保护与强制删除、任务提醒级别与统一排序规则、统计聚合口径一致性、演示数据自洽性。
