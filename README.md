...
| 模块 | 页面 | 主要能力 |
| --- | --- | --- |
| 养护总览 | `/dashboard` | 绿地与养护总量指标、近半年记录与工时趋势、绿地类型/任务类型/更换原因分布、逾期与临期任务提醒、养护工作量排名 |
| 绿地台账 | `/green-spaces` | 绿地建档（编号自动生成）、按行政区/类型/等级/状态/关键字检索、档案详情（概览 + 近期任务/记录/更换 + 更换原因汇总）、删除保护 |
| 养护任务 | `/tasks` | 任务登记（编号按日生成）、默认按紧急度（优先级 × 计划日期）排序、按状态/类型/优先级/绿地/计划日期区间/逾期筛选、状态流转（待执行→进行中→已完成/已取消）、任务详情与执行进度 |
...
│   │   │   ├── code_generator.py
│   │   │   ├── task_urgency.py  # 任务紧急度规则的 SQL 版本（排序/提醒共用）
...
8. **任务紧急度规则**（纯规则在 `utils/urgency.py`，SQL 版本在 `services/task_urgency.py`，任务列表默认排序、提醒清单、看板排名三处共用）：
   - 紧急度得分 = 优先级权重（低 1 ~ 紧急 4）× 25 + 临期分（逾期 ≥30 天 60 分、逾期 45 分、今日到期 30 分、3 天内 20 分、7 天内 10 分），得分高者排前，紧急且临期的任务最靠前；
   - 列表默认按得分降序，未完成任务在前、已关闭（完成/取消）任务在后；显式 `sort` 参数可覆盖默认排序；
   - 提醒周期按优先级区分：紧急 14 天、高 7 天、中 3 天、低 1 天——计划日期进入周期（或已逾期）的未完成任务进入提醒清单与看板「临期任务提醒」，总览的「提醒周期内」统计同口径；
   - 提醒级别按得分划分：≥130 紧急提醒、≥90 重点提醒、其余常规提醒，未进入提醒周期的任务不带级别。
...
```bash
cd backend
python -m pytest              # 60 个用例：接口、校验、跨模块规则、端到端流程
```

覆盖重点：绿地编号生成与唯一性、枚举与字段校验、列表过滤/排序/分页、任务状态自动流转与手动流转限制、记录删除后的状态回退、更换金额核算、删除保护与强制删除、统计聚合口径一致性、任务紧急度规则（排序/提醒周期/提醒级别三处同源）、演示数据自洽性。
